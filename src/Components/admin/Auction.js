...
function Post(props) {
  ...
  function getPost()
  {
    axios.get(`http://localhost/project9/PHP/api/API/adminpost.php`)
      // console.log(aa)
      .then((res) => {
        //  console.log(res)
        const info = res.data;
  ...
  const deletePost=(id)=>
  {
    axios.delete(`http://localhost/project9/PHP/api/API/deletepost.php/${id}/delete`)
    .then(function(res){
      ...
      <nav className="navv">
        ...
        <div className="app-view">
          <nav className="utility-bar">
            <div className="utility-bar__inner">
              ...
              <a href="#" className="utility-bar__account">
                ...
                <span className="utility-bar__account-name">ِadmin@admin.com</span>
              </a>
            </div>
          </nav>
          <main className="content">
            ...
            <Link to="/AddpostAdmin" className="bttn btn-add"><h2><span>Add</span></h2> </Link>
            {/* </div> */}
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                ...
                <tr className='td-data' key={info}>
                  ...
                  <th>{info.end_data}</th>
                  <td>
            <button className="btn" onClick={(e)=>{e.preventDefault();sessionStorage.setItem('postId',info.id); window.location=`/EditPostAdmin/${info.id}/edit`}}>Edit</button>
            {/* <Link to={`/EditpostAdmin/${info.id}/edit`} className="btn" style={{borderRadius:'12px'}}>Edit</Link> */}
            <button className="btn btn-delete" onClick={()=>deletePost(info.id)}>Delete</button>
            {/* <Link to="/AddpostAdmin" onClick={()=>deletepost(info.id)} className="btn btn-delete" style={{borderRadius:'12px'}}>Delete</Link> */}
                  </td>
                </tr>
                </>
          )}
